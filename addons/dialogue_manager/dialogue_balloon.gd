...
var dialogue_line: DialogueLine:
	set(value):
		if value:
			...
			# The dialogue has finished — emit signal before closing
...
func _unhandled_input(_event: InputEvent) -> void:
	# Block all input while confrontation is playing
	if _is_confrontation_playing():
		return
...
func _is_confrontation_playing() -> bool:
	var cinematic = get_tree().get_first_node_in_group("cinematic_manager")
	if cinematic and cinematic.get("is_playing_confrontation"):
		return cinematic.is_playing_confrontation
	return false
...
func _on_balloon_gui_input(event: InputEvent) -> void:
	# Block input during confrontation
	if _is_confrontation_playing():
		return
